...
final class HierarchicalCollection
{
    ...
    private function addInHierarchy(Route $route, string $parentPrefix = ''): bool
    {
        ...
        if (strlen($commonPrefix) <= strlen($parentPrefix)) {
            return false;
        }
        ...
        $nonSharedPrefix = substr($fullPrefix, strlen($commonPrefix));

        $this->commonPrefix = substr($commonPrefix, strlen($parentPrefix));
        ...
        $this->children = [new self($nonSharedPrefix, $this->children), $route];
